...
{
    partial class ProxyTypeHelper<T>
    {
        internal class CustomType : Type
        {
            ...
            {
                return CustomProperties.Values.ToArray();
            /*    return _baseType.GetProperties(bindingAttr)
                    .Concat(CustomProperties.Values).ToArray();*/
            }

            protected override PropertyInfo GetPropertyImpl(string name, BindingFlags bindingAttr, Binder binder, Type returnType, Type[] types, ParameterModifier[] modifiers)
            {

                PropertyInfo retVal = GetProperties(bindingAttr).FirstOrDefault(prop => prop.Name == name);


                //retVal = CustomProperties[name];

                if (retVal == null && CustomProperties.ContainsKey(name)) // && CustomProperties.TryGetValue(name, out retVal )
                    retVal = CustomProperties[name];
                    //CustomProperties.TryGetValue(name, out retVal);

                // Look for the CLR property with this name first.
                return retVal; 
                
                /*GetProperties(bindingAttr).FirstOrDefault(prop => prop.Name == name)
